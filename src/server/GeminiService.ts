...
import { GenerateContentRequest, GenerateContentResult } from '../shared/types';
...
declare const UrlFetchApp: {
  fetch(url: string, options: any): { getResponseCode(): number; getContentText(): string };
};
...
function getApiKey_(): string {
  ...
  if (!key) throw new Error('Gemini API Key is not set. Save it in user settings first.');
  return key;
}

function isRetryable_(code: number): boolean {
  // 429/5xx は再試行
  return code === 429 || (code >= 500 && code < 600);
}

export function generateContent(req: GenerateContentRequest): GenerateContentResult {
  ...
  if (typeof req.topP === 'number') payload.topP = req.topP;
  if (typeof req.topK === 'number') payload.topK = req.topK;
  ...
  while (attempt < maxAttempts) {
    ...
    if (code >= 200 && code < 300) {
      try {
        ...
        return { ok: false, error: `Invalid JSON from Gemini: ${e?.message || e}` };
      }
    }
